...
-  __secondary url__ `api/info/farm/`
...
- __croptype__ : Use this to find a specific croptype eg maize
...
- __farmers_type_crop__ : Pass in the `croptype  attribute and this attribute to find all farms that cultivates a particular crop type 
...
### Acess some analytical data for farm specific functions via the following endpoints 
-  __secondary url__ `api/info/crops/`
    - __cropype__ : Pass this to edit your request to a particular `croptype` when passed most analysis will consider the croptype 
    - __cropUuid__: Get a particular crop object by passing in the `uuid` filter your response with this uuid 
    - __farmUuid__: Get response with relationship with a particular farm by passing the farm `uuid`
    - __isReady__: Get information about crops that are ready (harvested) pass it the `isReady` with `croptype` attribute
    - __weightReady__: Get the weight of a particuar crop that is ready pass with `croptype` attribute
    - __weightExpected__: Get the weight of a crop that is under cultivation passing the `croptype` attribute
    - __isCultivated__ : Pass with a crop uuid to see if it is being cultivated 
